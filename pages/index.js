...
function calcETA(feeRate, fees) {
  if (!fees) return { t: '—', d: 'Unknown', c: '#555555' };
...
// ─────────────────────────────────────────
// CSS — valori diretti, NESSUNA css variable
// ─────────────────────────────────────────
const CSS = `
  ...
  async function check() {
    ...
    try {
      ...
      if (!chain) {
        ...
        if (!chain) throw new Error('Could not detect blockchain. Try a Bitcoin or Ethereum address.');
      }
      ...
      setResult({ chain, addr: a, addrData, fees: stats?.fees || null, btcConfirmed, btcInfo });
  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 780, margin: '0 auto', padding: '72px 32px 96px' }}>
        ...
        <div style={{ marginBottom: 52 }}>
          ...
          <h1 style={{ fontSize: 'clamp(30px,4.5vw,48px)', fontWeight: 600, lineHeight: 1.1, letterSpacing: -2, marginBottom: 16, color: '#f0f0f0' }}>
            When does your<br />
            <span style={{ color: '#555555' }}>crypto arrive?</span>
          </h1>
          <p style={{ fontSize: 15, color: '#999999', maxWidth: 460, lineHeight: 1.75, marginBottom: 40 }}>
          ...
          {/* SEARCH */}
          <div style={{ display: 'flex', border: '1px solid #2e2e2e', borderRadius: 8, overflow: 'hidden', background: '#111111', marginBottom: 10 }}>
            <input
              value={addr}
              ...
              autoComplete="off"
              spellCheck="false"
              style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: '#f0f0f0', fontFamily: 'JetBrains Mono, monospace', fontSize: 14, padding: '16px 20px', minWidth: 0 }}
            />
            <button
              onClick={check}
              disabled={loading}
          ...
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: '#555555' }}>
        ...
        <div style={{ marginBottom: 72 }}>
          ...
          {result  && !loading && <ResultCard result={result} chainInfo={chainInfo} stats={stats} />}
        </div>

        {/* FEATURES */}
        <div style={{ fontSize: 12, color: '#555555', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: 18 }}>
          Why eta21
        </div>
        <Features />
      ...
      <footer style={{ borderTop: '1px solid #242424', padding: '22px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ fontSize: 12, color: '#555555' }}>For informational purposes only</div>
      </footer>
    </>
  );
}
...
function StatsRow({ stats }) {
  ...
  const cells = [
    ...
    { label: '30 min fee',  value: stats?.fees ? stats.fees.halfHourFee + ' sat/vB' : '—', color: '#f0f0f0' },
    { label: '1 hour fee',  value: stats?.fees ? stats.fees.hourFee + ' sat/vB' : '—',     color: '#f0f0f0' },
    { label: 'Mempool txs', value: stats?.mempool ? Number(stats.mempool.count).toLocaleString() : '—', color: '#f0f0f0' },
    { label: 'BTC / USD',   value: stats?.price ? '$' + Number(stats.price.USD).toLocaleString() : '—', color: '#f0f0f0' },
  ];
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', border: '1px solid #242424', borderRadius: 8, overflow: 'hidden', marginBottom: 52 }}>
      {cells.map((c, i) => (
        <div key={i} style={{ padding: '18px 20px', borderRight: i < 4 ? '1px solid #242424' : 'none' }}>
          <div style={{ fontSize: 12, color: '#555555', marginBottom: 6 }}>{c.label}</div>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 14, fontWeight: 500, color: c.color }}>{c.value}</div>
        </div>
      ))}
    </div>
  );
...
function Spinner({ addr }) {
  return (
    <div style={{ padding: '56px 32px', textAlign: 'center', color: '#555555', fontSize: 14 }}>
...
function MsgBox({ icon, title, sub, isError }) {
  return (
    <div style={{ border: '1px solid #242424', borderRadius: 8, padding: '56px 32px', textAlign: 'center' }}>
      ...
      {sub && <div style={{ fontSize: 13, color: '#555555' }}>{sub}</div>}
    </div>
  );
}
...
function ChainBadge({ chain, chainInfo }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: '1px solid #2e2e2e', borderRadius: 4, padding: '4px 10px', fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#999999' }}>
...
function ResultCard({ result, chainInfo, stats }) {
  ...
  return (
    <div style={{ border: '1px solid #242424', borderRadius: 8, overflow: 'hidden', animation: 'up .2s ease' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '22px 26px', borderBottom: '1px solid #242424', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 500 }}>
              {pending ? `${mempool.length} pending transaction${mempool.length > 1 ? 's' : ''}` : 'No pending transactions'}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
              <ChainBadge chain={chain} chainInfo={chainInfo} />
              {isBTC && btcBalance !== null && (
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#555555' }}>
                  {fmtSats(btcBalance)}
                </span>
              )}
            </div>
        ...
        {isBTC && etaInfo ? (
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: 11, color: '#555555', textTransform: 'uppercase', letterSpacing: 1, marginTop: 3 }}>estimated arrival</div>
          </div>
        ) : pending ? (
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 22, fontWeight: 500, color: '#fbbf24' }}>Pending</div>
            <div style={{ fontSize: 11, color: '#555555', textTransform: 'uppercase', letterSpacing: 1, marginTop: 3 }}>in mempool</div>
          </div>
        ) : (
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 16, fontWeight: 500, color: '#4ade80' }}>All confirmed ✓</div>
            <div style={{ fontSize: 11, color: '#555555', textTransform: 'uppercase', letterSpacing: 1, marginTop: 3 }}>no pending</div>
          </div>
        )}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', borderBottom: '1px solid #242424' }}>
          {[
            ...
            <div key={i} style={{ padding: '18px 26px', borderRight: i < 3 ? '1px solid #242424' : 'none' }}>
              <div style={{ fontSize: 11, color: '#555555', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>{cell.l}</div>
      ...
      {pending && (
        <>
          <div style={{ padding: '14px 26px 10px', fontSize: 11, color: '#555555', textTransform: 'uppercase', letterSpacing: 1, borderBottom: '1px solid #242424' }}>
      ...
      {isBTC && pending && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 26px', gap: 16, flexWrap: 'wrap', background: '#161616', borderTop: '1px solid #242424' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#555555' }}>Accelerate via mining pools — priority confirmation</div>
          </div>
          <a
            href={`https://mempool.space/tx/${mempool[0]?.txid}#accelerate`}
            target="_blank" rel="noreferrer"
            style={{ background: '#7c6cfa', color: '#ffffff', borderRadius: 6, padding: '10px 20px', fontSize: 13, fontWeight: 500, textDecoration: 'none', whiteSpace: 'nowrap' }}
          >
            Accelerate →
          </a>
        </div>
      ...
      {/* NO PENDING */}
      {!pending && (
        <div style={{ padding: '20px 26px', color: '#555555', fontSize: 13, fontFamily: 'JetBrains Mono, monospace', borderTop: '1px solid #242424' }}>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 26px', borderBottom: isLast ? 'none' : '1px solid #242424', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
        ...
        <div>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 13, color: '#999999' }}>
            <a href={explorerUrl} target="_blank" rel="noreferrer">{trunc(txid)}</a>
          </div>
          <div style={{ fontSize: 11, color: '#555555', marginTop: 3, fontFamily: 'JetBrains Mono, monospace' }}>{meta}</div>
        </div>
      </div>
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 13, fontWeight: 500, color: amtColor }}>{amount}</div>
        <div style={{ fontSize: 11, color: '#555555', marginTop: 3, fontFamily: 'JetBrains Mono, monospace' }}>{status}</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', border: '1px solid #242424', borderRadius: 8, overflow: 'hidden', marginBottom: 72 }}>
      {feats.map((f, i) => (
        <div key={i} style={{
          ...
          <div style={{ fontSize: 13, color: '#555555', lineHeight: 1.7 }}>{f.desc}</div>
        </div>
      ))}
    </div>
  );
