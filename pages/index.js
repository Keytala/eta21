  ...
  return (
    <>
      ...
      <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',height:56,padding:'0 32px',borderBottom:'1px solid #2a2a2a',position:'sticky',top:0,background:'rgba(15,15,15,.96)',backdropFilter:'blur(20px)',zIndex:99}}>
        <div style={{fontFamily:'JetBrains Mono, monospace',fontSize:16,fontWeight:500,letterSpacing:'-.5px'}}>
        ...
        <div style={{display:'flex',gap:24,fontSize:13,color:'#888'}}>
          ...
          <a href="/api-docs" style={{color:'#888'}}>API</a>
        </div>
      </header>
      ...
      <main style={{maxWidth:680,margin:'0 auto',padding:'80px 24px 60px',textAlign:'center'}}>
        ...
        <div style={{display:'inline-flex',alignItems:'center',gap:6,border:'1px solid #2a2a2a',borderRadius:4,padding:'5px 12px',fontFamily:'JetBrains Mono, monospace',fontSize:11,color:'#888',marginBottom:32}}>
          <span style={{width:5,height:5,borderRadius:'50%',background:'#4ade80',boxShadow:'0 0 5px #4ade80',display:'inline-block',animation:'blink 2s ease-in-out infinite'}} />
          Bitcoin · Ethereum · Solana · +45 chains
        </div>

        <h1 style={{fontSize:'clamp(28px,5vw,52px)',fontWeight:600,lineHeight:1.1,letterSpacing:-2,marginBottom:20,color:'#f0f0f0'}}>
          Is your crypto<br />
          <span style={{color:'#7c6cfa'}}>wallet safe?</span>
        </h1>

        <p style={{fontSize:16,color:'#888',maxWidth:460,margin:'0 auto 48px',lineHeight:1.75}}>
        ...
        {/* SEARCH */}
        <div style={{display:'flex',border:'1px solid #333',borderRadius:8,overflow:'hidden',background:'#161616',maxWidth:560,margin:'0 auto 12px'}}>
          <input
            value={addr}
            ...
            autoComplete="off"
            spellCheck="false"
            style={{flex:1,background:'none',border:'none',outline:'none',color:'#f0f0f0',fontFamily:'JetBrains Mono, monospace',fontSize:13,padding:'16px 20px',minWidth:0}}
          />
          <button
            onClick={check}
            disabled={loading || !addr.trim()}
            style={{background: addr.trim() ? '#7c6cfa' : '#1c1c1c',color: addr.trim() ? '#fff' : '#444',border:'none',padding:'0 28px',fontFamily:'Inter, sans-serif',fontSize:14,fontWeight:500,cursor: addr.trim() ? 'pointer' : 'not-allowed',whiteSpace:'nowrap',transition:'background .15s'}}
          >
            {loading ? 'Loading…' : 'Check →'}
          </button>
        </div>

        <div style={{fontSize:12,color:'#444',fontFamily:'JetBrains Mono, monospace',marginBottom:64}}>
          Read-only · No sign-up · Your address is never stored
        </div>

        {/* STATS */}
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:1,border:'1px solid #2a2a2a',borderRadius:8,overflow:'hidden',marginBottom:80}}>
          {[
            { n: '48+',    l: 'Blockchains supported' },
            { n: '6',      l: 'Risk categories analyzed' },
            { n: '100',    l: 'Point scoring system' },
          ].map((s,i) => (
            <div key={i} style={{padding:'24px 20px',background:'#161616',borderRight: i<2 ? '1px solid #2a2a2a' : 'none',textAlign:'center'}}>
        ...
        <div style={{textAlign:'left',marginBottom:80}}>
          <div style={{fontSize:11,color:'#444',textTransform:'uppercase',letterSpacing:'1.5px',marginBottom:20,textAlign:'center'}}>How it works</div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:1,border:'1px solid #2a2a2a',borderRadius:8,overflow:'hidden'}}>
            {[
              ...
              <div key={i} style={{padding:'22px 24px',background:'#161616',borderRight: i%2===0 ? '1px solid #2a2a2a' : 'none',borderBottom: i<4 ? '1px solid #2a2a2a' : 'none'}}>
                <div style={{fontSize:18,marginBottom:10}}>{f.icon}</div>
                <div style={{fontSize:13,fontWeight:500,marginBottom:5}}>{f.t}</div>
                <div style={{fontSize:12,color:'#888',lineHeight:1.6}}>{f.d}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{textAlign:'left'}}>
          <div style={{fontSize:11,color:'#444',textTransform:'uppercase',letterSpacing:'1.5px',marginBottom:20,textAlign:'center'}}>Score examples</div>
          <div style={{border:'1px solid #2a2a2a',borderRadius:8,overflow:'hidden'}}>
            {[
              ...
              <div key={i} style={{display:'flex',alignItems:'center',gap:16,padding:'16px 20px',borderBottom: i<3 ? '1px solid #2a2a2a' : 'none',background:'#161616'}}>
                <div style={{fontFamily:'JetBrains Mono, monospace',fontSize:20,fontWeight:600,color:e.color,minWidth:40,textAlign:'right'}}>{e.score}</div>
                <div style={{flex:1}}>
      ...
      <footer style={{borderTop:'1px solid #2a2a2a',padding:'20px 32px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
        ...
        <div style={{fontSize:11,color:'#444'}}>For informational purposes only · Not financial advice</div>
      </footer>
    </>
  );
}
